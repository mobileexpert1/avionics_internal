...
class UnitSelectionCubit extends Cubit<UnitSelectionState> {
  ...
  Future<void> getUnitPreferences(BuildContext context) async {
    ...
    emit(
      UnitSelectionInitial(
        ...
        altitude: currentState is UnitSelectionInitial ? currentState.altitude : '',
        distance: currentState is UnitSelectionInitial ? currentState.distance : '',
        temperature: currentState is UnitSelectionInitial ? currentState.temperature : '',
        isLoading: true,
        isSuccess: false,
    ...
    try {
      ...
      final selectedSpeed = response.speed.firstWhere((e) => e.isSelected, orElse: () => response.speed.first).unit;
      final selectedAltitude = response.altitude.firstWhere((e) => e.isSelected, orElse: () => response.altitude.first).unit;
      final selectedDistance = response.distance.firstWhere((e) => e.isSelected, orElse: () => response.distance.first).unit;
      final selectedTemperature = response.temperature.firstWhere((e) => e.isSelected, orElse: () => response.temperature.first).unit;
  ...
  Future<void> submitPreferences(BuildContext context) async {
    ...
    try {
      ...
      final selectedSpeed = result.speed.firstWhere((e) => e.isSelected, orElse: () => result.speed.first).unit;
      final selectedAltitude = result.altitude.firstWhere((e) => e.isSelected, orElse: () => result.altitude.first).unit;
      final selectedDistance = result.distance.firstWhere((e) => e.isSelected, orElse: () => result.distance.first).unit;
      final selectedTemperature = result.temperature.firstWhere((e) => e.isSelected, orElse: () => result.temperature.first).unit;
