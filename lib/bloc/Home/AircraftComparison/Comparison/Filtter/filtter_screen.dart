        ...
        actions: [
          BlocBuilder<ComparisonFilterCubit1, FilterState1>(
            builder: (context, state) {
              final selectedCount = state.filterCategories.fold<int>(
                0,
                    (previousValue, category) => previousValue + category.options.where((option) => option.isSelected).length,
              );
              return TextButton(
                onPressed: () {
                  final cubit = context.read<ComparisonFilterCubit1>();
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final List<FilterOption1> optionsToDisplay = category.isExpanded
                        ? category.options
                        : category.options.take(visibleOptionCountWhenCollapsed).toList();
                    ...
                    return Column(
                      ...
                      children: [
                                ...
                                children: [
                                  ...
                                  TextButton(
                                    ...
                                    child: Row(
                                      children: [
                                        Text(
                                          category.isExpanded ? 'Show Less' : 'Show More',
                                          style: TextStyle(color: Colors.grey),
                                        ),
                                        const SizedBox(width: 5),
                                        SvgPicture.asset(
                                          CommonUi.setSvgImage(
                                            category.isExpanded ? AssetsPath.upArrow : AssetsPath.downArrow,
                                          ),
                                          fit: BoxFit.fill,
                                          width: 16,
                                          height: 16,
                        ...
                        else if (optionsToDisplay.isNotEmpty)
                          ...optionsToDisplay.map((option) {
                            return Padding(
