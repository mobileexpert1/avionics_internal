...
import 'chat_model.dart';                // ChatAuthor / ChatMessage

class ChatCubit extends Cubit<List<Map<String, String>>> {
  ChatCubit({required String accessToken}): _repo = ChatRepositoryImpl(),
      super(const [
        {'type': 'bot', 'text': 'Hey there!'},
        {'type': 'bot', 'text': 'I’m your WILCO, How can I help you?'},
        {'type': 'button', 'text': ''},
  ...
  Future<void> _init(String token) async {
    // 1. open (or resume) the socket
    await _repo.connect(accessToken: token);

    // 2. forward every incoming ChatMessage → UI format
    _sub = _repo.messages.listen((chatMsg) {
      emit([
        ...state,
        {
          'type': chatMsg.author == ChatAuthor.user ? 'user' : 'bot',
          'text': chatMsg.text,
        },
      ]);
    });
  ...
  // Called by your Send-button
  void sendMessage(String text) => _repo.send(text);
